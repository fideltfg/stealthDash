 ...
 * Docker Plugin
 *
 * Provides Docker container management API.
 * Supports listing containers, start/stop/restart, and viewing logs.
 */
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (credentialId && isHttps) {
        try {
          const [credential] = await db.query('SELECT * FROM credentials WHERE id = ?', [credentialId]);
...
router.post('/api/docker/containers', authMiddleware, async (req, res) => {
  try {
...
router.post('/api/docker/containers/start', authMiddleware, async (req, res) => {
  try {
...
router.post('/api/docker/containers/stop', authMiddleware, async (req, res) => {
  try {
    const { host, credentialId, containerId } = req.body;
...
router.post('/api/docker/containers/restart', authMiddleware, async (req, res) => {
  try {
    const { host, credentialId, containerId } = req.body;
...
router.post('/api/docker/containers/logs', authMiddleware, async (req, res) => {
  try {
...
module.exports = {
  name: 'docker',
  description: 'Docker container management API (list, start, stop, restart, logs)',
  version: '1.0.0',
  routes: router,
  mountPath: '/',
};
