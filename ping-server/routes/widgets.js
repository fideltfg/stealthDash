...
router.post('/api/docker/containers', async (req, res) => {
  const { host, credentialId } = req.body;
  ...
  try {
    // Get credentials if credentialId provided
    let dockerConfig = {};
    if (credentialId) {
      const authHeader = req.headers.authorization;
      if (!authHeader || !authHeader.startsWith('Bearer ')) {
    ...
    if (host.startsWith('unix://')) {
      // Unix socket - not supported via HTTP proxy, needs direct socket connection
      return res.status(400).json({ 
        error: 'Unix socket not supported yet. Please use HTTP/HTTPS Docker host.' 
      });
    }
    
    // Ensure URL has proper format
    if (!dockerUrl.startsWith('http')) {
      dockerUrl = `http://${dockerUrl}`;
    }
    
    const containersUrl = `${dockerUrl}/containers/json?all=${showAll}`;
    ...
    // Add TLS cert if provided in credentials
    if (dockerConfig.cert && dockerConfig.key) {
      // For TLS authentication - would need https.Agent with cert/key
      // This is a placeholder - full implementation would need node https module
    }

    const response = await fetch(containersUrl, requestOptions);
...
router.post('/api/docker/containers/start', async (req, res) => {
  ...
  if (!host || !containerId) {
    return res.status(400).json({ error: 'host and containerId are required' });
  }
  ...
  try {
    ...
    if (credentialId) {
      ...
      const jwt = require('jsonwebtoken');
      const decoded = jwt.verify(authToken, process.env.JWT_SECRET || 'your-secret-key-change-in-production');
      await getCredentials(credentialId, decoded.userId);
    }
    ...
    if (!response.ok && response.status !== 304) { // 304 = already started
      const errorText = await response.text();
      throw new Error(`Failed to start container: ${response.status} ${errorText}`);
    }
    
    res.json({ success: true, action: 'start', containerId });
...
router.post('/api/docker/containers/stop', async (req, res) => {
  ...
  try {
    ...
    const stopUrl = `${dockerUrl}/containers/${containerId}/stop`;
    ...
    res.json({ success: true, action: 'stop', containerId });
...
router.post('/api/docker/containers/restart', async (req, res) => {
  ...
  try {
    ...
    const restartUrl = `${dockerUrl}/containers/${containerId}/restart`;
    ...
    res.json({ success: true, action: 'restart', containerId });
